...
snippet_spdoinfo = "snippet/spdoinfo.txt"
snippet_spref = "snippet/spref.txt"
...
def runProcess(fname, fpath):
	...
	with open(fpath, encoding="utf-8") as xmlfile:
		...
		try:
			...
			spiIndex = root.index(spdoinfo)
			root.remove(spdoinfo)
			with open(snippet_spdoinfo, encoding="utf-8") as spiSnippet:
				spi = et.parse(spiSnippet)
				spiRoot = spi.getroot()
				root.insert(spiIndex, spiRoot)
		except Exception as e:
			errorMsg(e)

		announce(fname, "Replace spref")
		try:
			spref = root.find(".//spref")
			sprIndex = root.index(spref)
			root.remove(spref)
			with open(snippet_spref, encoding="utf-8") as sprSnippet:
				spr = et.parse(sprSnippet)
				sprRoot = spr.getroot()
				root.insert(sprIndex, sprRoot)
		except Exception as e:
			errorMsg(e)
